...
export class BattleActions {
			...
			if (move.ohko) { // bypasses accuracy modifiers
				...
				if (accuracy !== true) {
					...
					if (!move.ignoreAccuracy) {
						...
						boost = this.battle.clampIntRange(boosts['accuracy'], -1, 1);
					}
					if (!move.ignoreEvasion) {
						const boosts = this.battle.runEvent('ModifyBoost', target, null, null, {...target.boosts});
						boost = this.battle.clampIntRange(boost - boosts['evasion'], -1, 1);
					}
					if (boost > 0) {
	...
	hitStepMoveHitLoop(targets: Pokemon[], pokemon: Pokemon, move: ActiveMove) { // Temporary name
		...
		for (hit = 1; hit <= targetHits; hit++) {
			...
			if (hit > 1 && pokemon.status === 'drowsy' && (!isSleepUsable || this.battle.gen === 4)) break;
			...
			if (target && move.multiaccuracy && hit > 1) {
				...
				if (accuracy !== true) {
					if (!move.ignoreAccuracy) {
						const boosts = this.battle.runEvent('ModifyBoost', pokemon, null, null, {...pokemon.boosts});
						const boost = this.battle.clampIntRange(boosts['accuracy'], -1, 1);
						if (boost > 0) {
					...
					if (!move.ignoreEvasion) {
						const boosts = this.battle.runEvent('ModifyBoost', target, null, null, {...target.boosts});
						const boost = this.battle.clampIntRange(boosts['evasion'], -1, 1);
						if (boost > 0) {
							accuracy /= boostTable[boost];
						} else if (boost < 0) {
							accuracy *= boostTable[-boost];
