	...
	*/
	...
	setSpecies(rawSpecies: Species, source: Effect | null = this.battle.effect, isTransform = false) {
		...
		if (this.battle.gen <= 1) {
			...
			if (this.status === 'brn') this.modifyStat!('atk', 0.66);
			if (this.status === 'fsb') this.modifyStat!('spa', 0.66);
		}
	...
	cureStatus(silent = false) {
		...
		if (this.status === 'drowsy' && !this.hasAbility('comatose') && this.removeVolatile('nightmare')) {
