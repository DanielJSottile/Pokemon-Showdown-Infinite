...
export const Abilities: { [abilityid: string]: AbilityData } = {
	...
	deflagrate: {
		...
		onBasePowerPriority: 23,
		onBasePower(basePower, pokemon, target, move) {
			if (move.deflagrateBoosted) return this.chainModify([4915, 4096]);
		},
		name: "Deflagrate",
		rating: 4,
		num: -3,
	...
	gravitywell: {
		...
		onImmunity(type, pokemon) {
			if (type === "maelstrom") return false;
		},
		name: "Gravity Well",
		rating: 1,
		num: 0,
	...
	healer: {
		...
		onResidual(pokemon) {
			for (const allyActive of pokemon.adjacentAllies()) {
				if (allyActive.status && this.randomChance(1, 4)) {
		...
		onStart(pokemon) {
			...
			for (const target of pokemon.adjacentFoes()) {
				...
				if (
					...
					pokemon.getStat("spe", false, false) <
						target.getStat("spe", false, false)
				) {
	...
	mimicry: {
		onStart(pokemon) {
			if (this.field.terrain) {
				...
				if (
					pokemon.getTypes().join() === types.join() ||
			...
			for (const target of pokemon.adjacentFoes()) {
				...
				if (target.volatiles["substitute"]) {
					...
					this.add(
						"-immune",
						target,
						`[from] ability: ${target.getAbility().name}`
					);
				} else {
	...
	serenegrace: {
		...
		onModifyMove(move) {
			if (move.secondaries) {
				this.debug("1.5x secondary chance");
	...
	sheerforce: {
		...
		onBasePowerPriority: 21,
		onBasePower(basePower, pokemon, target, move) {
			if (move.hasSheerForce) return this.chainModify(1.2);
		},
		name: "Sheer Force",
		rating: 3.5,
		num: 125,
	...
	typeflux: {
		...
		onResidual(pokemon) {
			...
			const type = this.sample(
				possibleTypes.filter((t) => pokemon.types[0] !== t)
			);
			this.add(
				"-start",
				pokemon,
	...
	unownsspell: {
		...
		onResidual(pokemon) {
			if (
				pokemon.baseSpecies.baseSpecies !== "Unown" ||
				pokemon.transformed ||
				!pokemon.hp
			) { return; }
			if (
